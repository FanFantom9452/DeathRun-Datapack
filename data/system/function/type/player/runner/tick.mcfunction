...
#碰到傳送門 設置重生點
execute if score @s checkpoint.cd matches 1.. unless block ~ ~ ~ nether_portal run scoreboard players remove @s checkpoint.cd 1
execute if score @s checkpoint.cd matches 0 run scoreboard players reset @s checkpoint.cd
execute unless score @s checkpoint.cd matches -2147483648..2147483647 if block ~ ~ ~ minecraft:nether_portal run function system:type/player/runner/set_checkpoint
#dash 物品
...
#用時計算
execute if score timer status matches 0 run function system:type/player/runner/reset_time
execute if score timer status matches 1 if score @s rank matches ..0 run function system:type/player/runner/use_time
